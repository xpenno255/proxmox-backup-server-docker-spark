...
RUN mkdir -p /tmp/pbs-packages && cd /tmp/pbs-packages && \
    ...
    curl -fSL -O "${RELEASE_URL}/proxmox-widget-toolkit_5.2.3_all.deb" && \
    curl -fSL -O "${RELEASE_URL}/pve-xtermjs_6.0.0-1_all.deb" && \
    curl -fSL -O "${RELEASE_URL}/pbs-i18n_3.7.4_all.deb" && \
